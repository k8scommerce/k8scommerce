...
type Category interface {
	Exists() bool
	Deleted() bool
	Create(prod *models.Category) error
	Update(prod *models.Category) error
	Save() error
	Upsert() error
	Delete(id int64) error
	GetCategoryById(CategoryByID int64) (res *categoryResponse, err error)
	GetCategoryBySlug(sku string) (res *categoryResponse, err error)
}
...
func (m *categoryRepo) GetCategoryBySlug(slug string) (res *categoryResponse, err error) {
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("error::GetCategoryBySlug::%s", err.Error())
	}
	...
	if len(result) > 0 {
		out.Category = result[0].Category
...
func (m *categoryRepo) GetCategoryById(CategoryByID int64) (res *categoryResponse, err error) {
	nstmt, err := m.db.PrepareNamed(`
		WITH p AS (
			SELECT 
				product.* 
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("error::GetCategoryById::%s", err.Error())
	}
...
func (m *categoryRepo) GetProductsByCategoryId(categoryID, currentPage, pageSize int64, sortOn string) (res *getByCategoryIDResponse, err error) {
...
func (m *categoryRepo) GetAllProducts(currentPage, pageSize int64) ([]*categoryRepo, error) {
	return nil, nil
}

func (m *categoryRepo) Create(prod *models.Category) error {
...
func (m *categoryRepo) Update(prod *models.Category) error {
	if prod.ID == 0 {
...
func (m *categoryRepo) Delete(id int64) error {
	prod, err := models.CategoryByID(m.ctx, m.db, id)
	if err != nil {
		return err
	}
...
// func (m *categoryRepo) getProductVariants(CategoryByID int64) (*[]types.Variant, error) {
// 	nstmt, err := m.db.PrepareNamed(`
// 		SELECT
// 			*
// 		FROM variants
